...
| `SmokeBackground` | none | Cursor-driven WebGL2 fluid smoke on a fixed canvas (`z-index: -1`, `transition:persist`) — mounted in `BaseLayout` but active only on the homepage feed (`/`, `/2/`, …); it switches itself on/off on `astro:page-load` and creates its GL context on the first homepage visit. Runs only for a fine pointer, no reduced-motion, and WebGL2 + `EXT_color_buffer_float`; pauses on hidden tabs and stops drawing ~9s after the mouse goes still. Tuning constants sit at the top of its script |
